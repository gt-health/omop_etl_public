...
*/
CREATE OR REPLACE procedure omop.generic_create_drug_exposures( v_loadid int)
...
-- drug exposure from pharmacy
 ...
 etl.logm('generic_create_drug_exposures', 'create drug_exposures', 'START' );
...
(
    select
    	...
    	coalesce(cast(s.drug_source_type_value as number),0) as drug_type_concept_id,
...
;
  ...
  rowcnt := SQL%ROWCOUNT;
  etl.logm('generic_create_drug_exposures', 'insert into drug_exposure', rowcnt );
  etl.logm('generic_create_drug_exposures', 'create drug_exposure', 'FINISH' );
