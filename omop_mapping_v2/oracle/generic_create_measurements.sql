...
*/
CREATE OR REPLACE procedure omop.generic_create_measurements(v_loadid number)
...
begin
    ...
    etl.logm('generic_create_measurements', 'create measurements', 'START' );
    ...
    (
     ...
     (
        select
           ...
           as measurement_time,
           coalesce( cast(s.measurement_source_type_value as number),0)  as measurement_type_concept_id,    -- add to stage table
           coalesce( oper.concept_id, 0 ) as operator_concept_id,
           cast(s.value_as_number as number ) as value_as_number,
    ...
    rowcnt := SQL%ROWCOUNT;
    etl.logm('generic_create_measurements', 'insert into measurements', rowcnt );
    ...
    (
      select 
          ...
          trim( to_char(extract( hour from s.measurement_date ), '09') ) || ':' ||
            ...
            trim( to_char(extract( second from s.measurement_date ), '09') )
          as observation_time,
          coalesce( cast( s.measurement_source_type_value as number), 0 )    as observation_type_concept_id,
          cast( s.value_as_number as number ) as value_as_number,
    ...
    etl.logm('generic_create_measurements', 'create measurements', 'FINISH' );
