...
CREATE OR REPLACE
PROCEDURE omop.generic_create_locations (  v_loadid INT )
...
BEGIN
  ...
  etl.logm('generic_create_locations', 'generic_create_locations' , 'START' );
  ...
  SELECT 
    omop.location_id_seq.nextval AS location_id ,
  ...
  FROM
    ...
    FROM
      ...
      WHERE p.load_id = COALESCE(v_loadid, p.load_id )
      ...
      WHERE pr.load_id = COALESCE(v_loadid, pr.load_id )
  ...
  etl.logm('generic_create_locations', 'insert into location' , rowcnt );
  etl.logm('generic_create_locations', 'generic_create_locations' , 'FINISH' );
