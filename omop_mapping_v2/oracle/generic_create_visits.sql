...
*/
    ...
    create or replace procedure omop.generic_create_visits(v_loadid int)
    ...
    begin
         ...
         etl.logm('generic_create_visits', 'generic_create_visits' , 'START' );
            ...
            select
                omop.VISIT_OCCURRENCE_ID_seq.nextval as visit_occurrence_id
                , case vis.visit_type
                    when 'IP' then 9201
                    when 'OUT' then 9202
                    when 'ER' then 9203
       ...
       and vis.load_id = coalesce(v_loadid, vis.load_id )
;


          rowcnt := SQL%ROWCOUNT;

          etl.logm('generic_create_visits', 'insert into visit_occurrence' , rowcnt );

          etl.logm('generic_create_visits', 'generic_create_visits' , 'FINISH' );
