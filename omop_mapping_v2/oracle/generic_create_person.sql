...
-- create person records
    ...
    create or replace procedure omop.generic_create_person(v_loadid int)
    ...
    begin
        ...
        etl.logm('generic_create_person', 'generic_create_person' , 'START' );
        ...
        select
            omop.person_id_seq.nextval as person_id,
            case pat.gender
                when 'M' then 8507
                when 'F' then 8532
                when 'A' then 8570
		...
		and pat.load_id = coalesce(v_loadid, pat.load_id )
        ;


          rowcnt := SQL%ROWCOUNT;

          etl.logm('generic_create_person', 'insert into person' , rowcnt );

          etl.logm('generic_create_person', 'generic_create_person' , 'FINISH' );
